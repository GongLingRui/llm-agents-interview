...
# 模型量化
通过降低模型精度（8BIT ,4bit），减少现存占用并且加速推理
...
```
# 网络优化
在多gpu或者分布式场景下，网络优化尤为重要
## 模型分片
将模型不同的部分分布到多台设备上，提升并行效率
...
- 量化有限：有限采用量化技术减少资源需求，
- 批处理优化：合理配置批处理大小，平衡延迟和吞吐
- 缓存策略:实施多层缓存，提升整体性能
- 监控调优：持续监控关键指标，动态调整推理参数
- ab测试：对比不同的优化方案，选择最佳的配置
- 
